        ...
        drawer: BlocBuilder<ChatBloc, AppState>(
          builder: (ctx, state) {
            if (state is ChatLoadingState) {
              ...
              return Drawer(child: MobileImageSideBar(ctx, state));
            } else if (state is ChatLoadedState) {
              return Drawer(child: MobileChatSidebar(ctx, state));
            } else if (state is UnderProgressState) {
              return Drawer(child: MobileUPChatSidebar(ctx, state));
            } else {
              return Center(
                child: CircularProgressIndicator(),
              );
                  ...
                  child: BlocBuilder<ChatBloc, AppState>(
                    builder: (ctx, state) {
                      ...
                      if (state is ImageGenerationState) {
                        return ImageContent(ctx, state);
                      }
                      if (state is ChatLoadedState) {
                        return ChatContent(ctx, state);
                      }
                      return Center(
                        child: CircularProgressIndicator(),
                      );
