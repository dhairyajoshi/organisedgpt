...
class DesktopChatScreen extends StatelessWidget {
  ...
  double temp = 0;
  BuildContext ctx;
  DesktopChatScreen(this.ctx, {super.key});
  @override
    ...
    return Scaffold(
      body: Container(
        ...
        child: Row(
          children: [
            Flexible(
                flex: MediaQuery.of(context).size.width > 1230 ? 3 : 4,
                fit: FlexFit.tight,
                child: BlocBuilder<ChatBloc, AppState>(
                  builder: (ctx, state) {
                    if (state is ChatLoadingState) {
                      ...
                      return ChatSidebar(ctx, state);
                    } else if (state is ImageGenerationState) {
                      return ImageSideBar(ctx, state);
                    } else if (state is UnderProgressState) {
                      return UPChatSidebar(ctx, state);
                    } else {
                      return Center(
                        child: CircularProgressIndicator(),
                      );
